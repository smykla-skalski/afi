...
**Four codes are the endpoint having a bad day, not a credential to fix.** `Throttling`, `ThrottlingException`, and `RequestLimitExceeded` are AWS shedding load; `IDPCommunicationError` is AWS failing to reach the identity provider registered on the role. All four report as `provider_http` with the status attached, so a job that re-runs on that kind gets its second attempt, and none of them sends anybody auditing a trust policy that was never the problem. STS answers them with a 400 rather than the 429 a modern API would use, which is why the code decides this and not the status. A 429 or a 5xx keeps its status and stays retryable as well.

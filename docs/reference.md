...
The tags are only honoured before the answer begins, which is where reasoning goes and the only place a model means them as markers. A reply *about* `<think>` - this paragraph, say - is passed through untouched, and so is a tag arriving after any content has streamed.

...
fn run_afi(home: &TempDir, addr: SocketAddr, prices: Option<&str>) -> Output {
...
fn a_priced_run_reports_what_it_cost() {
    ...
    let priced = summary_of(&run_afi(&home, addr, Some(RATES)));
    ...
    let unpriced = summary_of(&run_afi(&home, addr, None));

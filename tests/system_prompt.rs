...
use common::{repl_afi_in, run_afi_in, session_of, stderr_of};

/// One-shot afi, against `addr` when there is one and a closed port when there is not.
fn run_afi(home: &TempDir, addr: Option<SocketAddr>, extra: &[&str]) -> Output {
    run_afi_with_env(home, addr, extra, &[])
}

/// The same, with extra variables in the environment.
fn run_afi_with_env(
    home: &TempDir,
    ...
    run_afi_in(home, Path::new("."), addr, extra, env)
}
